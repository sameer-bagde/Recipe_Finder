...
describe('Recipe_Finder', () => {
  ...
  describe("SignupForm", () => {
    ...
    it('signup with existing email', () => {
      cy.window().then((win) => {
        win.localStorage.setItem('users', JSON.stringify([{ email: 'john@example.com' }]));
      });
  ...
  describe("SigninForm", () => {
    ...
    it("redirects to home page on successful sign-in", () => {
      cy.window().then((win) => {
        win.localStorage.setItem('users', JSON.stringify([{ email: "johndoe@example.com", password: "password123" }]));
      });
  ...
  describe('Favorites', () => {
    ...
    it('adds recipe to favorites when user is authenticated', () => {
      ...
      cy.intercept('GET', '**/recipes/123/information*', mockRecipeDetails).as('getRecipeDetails');

      cy.window().then((win) => {
        win.localStorage.setItem('authToken', 'test-token');
        win.localStorage.setItem('userData', JSON.stringify({ email: 'test@example.com' }));
        win.localStorage.setItem('users', JSON.stringify([{ email: 'test@example.com', favorites: [] }]));
      });
      ...
      cy.window().then((win) => {
        ...
        const currentUser = users.find((user) => user.email === 'test@example.com');
